...
<head>
    ...
    <title>الزيارات والكشف الطبي - LabCare</title>
...
<body data-page="visits">
    ...
    <aside class="sidebar" id="sidebar">
        <div class="sidebar-header">
            ...
            <div>
                <h2>LabCare</h2><span>نظام إدارة العيادات</span>
            </div>
        </div>
        ...
        <div class="modal" style="width:620px; background:#fff; padding:24px; border-radius:8px;">
            ...
            <div class="modal-body">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="bloodPressureInput" placeholder="120/80"
                            style="width:100%;padding:10px;border:1px solid #ccc;border-radius:4px;">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="0.1" id="temperatureInput" placeholder="37"
                            style="width:100%;padding:10px;border:1px solid #ccc;border-radius:4px;">
                    </div>
                </div>
            ...
            <div class="modal-footer" style="display:flex; justify-content:flex-end; gap:8px; margin-top:16px;">
                ...
                <button class="btn btn-primary" onclick="saveItem()"
                    style="padding:8px 16px; background:blue; color:#fff; border:none; border-radius:4px; cursor:pointer;">حفظ</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // دوال التحكم بالمودال لتجنب أخطاء الـ Undefined
        ...
        async function loadSelects() {
            try {
                const patientsRes = await fetch(`${API_BASE_URL}/persons`);
                if (!patientsRes.ok) throw new Error('خطأ في جلب المرضى');
                const patients = await patientsRes.json();
                const patientSel = document.getElementById('patientInput');
                patientSel.innerHTML = '<option value="">اختر المريض</option>' +
                    patients.map(p => `<option value="${p.id}">${p.full_name || p.name} (${p.national_id || ''})</option>`).join('');

                const usersRes = await fetch(`${API_BASE_URL}/users`);
                if (!usersRes.ok) throw new Error('خطأ في جلب المستخدمين');
                const users = await usersRes.json();
                const doctors = users.filter(u => u.role === 'doctor');

                const doctorSel = document.getElementById('doctorInput');
                doctorSel.innerHTML = '<option value="">اختر الطبيب</option>' +
                    doctors.map(d => `<option value="${d.id}">${d.person ? d.person.full_name : 'طبيب #' + d.id}</option>`).join('');
            } catch (error) {
        ...
        async function fetchVisits() {
            try {
                ...
                if (!res.ok) throw new Error('خطأ في جلب الزيارات');
        ...
        function renderTable() {
            ...
            const items = visitsData.filter(i => {
                const pName = (i.person ? (i.person.full_name || i.person.name) : '').toLowerCase();
                return pName.includes(q);
            });

            const tbody = document.getElementById('tableBody');
            if (!items.length) {
                tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:20px;">لا توجد زيارات مسجلة</td></tr>';
                return;
            }
        ...
        <tr>
          ...
          <td><strong>${item.person ? (item.person.full_name || item.person.name) : '---'}</strong></td>
          <td>${item.doctor && item.doctor.person ? item.doctor.person.full_name : '---'}</td>
          <td>${vitalsText(item)}</td>
          <td><span>${STATUS_TEXT[item.status] || item.status}</span></td>
          <td>
            <button onclick="deleteItem('${item.id}')" style="background:red;color:#fff;border:none;padding:4px 8px;border-radius:4px;cursor:pointer;">حذف</button>
          </td>
        </tr>
      `).join('');
        }
        ...
        async function saveItem() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    alert('فشل الحفظ، تأكد من صحة المدخلات');
                }
            } catch (e) {
                console.error(e);
